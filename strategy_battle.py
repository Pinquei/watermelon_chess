...
def AI_Score_Yellow_Side(board):
    ...
    for C0C1C2C3 in board[red_player]:
        if C0C1C2C3 != 0:
            if fun_Alive(fun_Group(C0C1C2C3)) == False:
                score = (score + 999)
    for C4C5 in board[yellow_player]:
        for NeighborC4C5 in Neighbor(C4C5):
            if Exist_Chess(NeighborC4C5) == 0:
                score = (score + 0)
            elif Exist_Chess(NeighborC4C5) == 1:
                score = (score + 99)
            else:
                score = (score + 100)
    return score
